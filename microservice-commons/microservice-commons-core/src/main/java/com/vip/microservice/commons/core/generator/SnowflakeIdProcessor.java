...
/**
 * @author echo
 * @title: SnowflakeIdProcessor
 * @date 2023/3/15 17:38
 */
public class SnowflakeIdProcessor {
     ...
     */
    public SnowflakeIdProcessor(long startTimestamp, long dataCenterId, long machineId) {
        long currentTimestamp = getCurrentTimestamp();
        if (startTimestamp > currentTimestamp) {
            throw new BusinessException(String.format("Start timestamp can't be greater than %d", currentTimestamp));
        }

        if (dataCenterId > MAX_DATA_CENTER_ID || dataCenterId < 0) {
            throw new BusinessException(String.format("Data center id can't be greater than %d or less than 0", MAX_DATA_CENTER_ID));
        }

        if (machineId > MAX_MACHINE_ID || machineId < 0) {
            throw new BusinessException(String.format("Machine id can't be greater than %d or less than 0", MAX_MACHINE_ID));
        }
    ...
    public String[] nextIds(int count) {
        if (count <= 0 || count > MAX_BATCH_COUNT) {
            throw new BusinessException(String.format("Count can't be greater than %d or less than 0", MAX_BATCH_COUNT));
        }
    ...
    public static void main(String[] args) {
        SnowflakeIdProcessor processor = new SnowflakeIdProcessor(1483200000000L, 2, 3);

        for (int i = 0; i < (1 << 12); i++) {
            System.out.println(processor.nextId());
        }
    }
}
